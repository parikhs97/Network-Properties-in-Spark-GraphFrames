...
def closeness(g):
    ...
    allPathLengths = g.shortestPaths(vertices)

	# Break up the map and group by ID for summing
    MapAndGroup = allPathLengths.select(functions.explode('distances'))
    MapAndGroup = MapAndGroup.groupBy('key')
	# Sum by ID
    summation = MapAndGroup.agg(functions.sum('value').alias('s'))
	# Get the inverses and generate desired dataframe.
    result = summation.selectExpr('key as id','1/s as closeness')     
