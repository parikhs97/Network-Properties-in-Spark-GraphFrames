...
def simple(g):
    ...
    # YOUR CODE HERE
    flippedEdges = g.edges.selectExpr("src as dst","dst as src")
    flippedEdges = flippedEdges.select("src","dst")
    ...
    # YOUR CODE HERE
    edges = g.edges.unionAll(flippedEdges)
    edges = edges.distinct().where("src != dst")
    vertices = g.vertices
    new_g = GraphFrame(vertices,edges) 
    return new_g
...
def degreedist(g):
    ...
    count = count.groupBy('degree').count()
    return count
...
def readFile(filename, large, sqlContext=sqlContext):
    ...
    # YOUR CODE HERE
    ExtractedEdges = lines.map(lambda x: x.split(delim))
    ExtractedVertices = lines.flatMap(lambda x: x.split(delim)).map(lambda x: (x,))
    ...
    # graphframe vertices
    # YOUR CODE HERE
    edges = sqlContext.createDataFrame(ExtractedEdges, ['src', 'dst']).distinct()    
    vertices = sqlContext.createDataFrame(ExtractedVertices, ['id']).distinct()
